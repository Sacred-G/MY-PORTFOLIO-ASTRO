...
const $$ProjectItem = createComponent(($$result, $$props, $$slots) => {
  ...
  `, "class")}> <a${addAttribute(`#projects/${project.id}`, "href")} class="project__link"> <div class="project__link-wrapper"> ${project.hasThumb && renderTemplate`<img${addAttribute(project.thumbPath, "src")}${addAttribute(project.name, "alt")}>`} <div${addAttribute(`project__base project__base--color-${project.color}`, "class")}> <h4 class="project__base-title">${project.name}</h4> </div> </div> </a> <div class="project__complete"> <div class="project__complete-wrapper clear--after"> ${project.images?.length > 0 && renderTemplate`<div class="project__slides"> <div class="project__slides-wrapper"> ${project.images.map((imgSrc) => renderTemplate`<div class="project__slide"> <img${addAttribute(`${imgSrc}`, "data-lazyload")}> </div>`)} </div> </div>`} <div class="project__infos"> <h4 class="project__title">${project.name}</h4> <div class="project__desc"> ${project.desc?.en?.map(($paragraph) => renderTemplate`<p>${$paragraph}</p>`)} ${project.area && renderTemplate`<p> <strong>Area:</strong>: ${project.area} </p>`} </div> ${project.url && project.url.length > 4 && renderTemplate`<div class="project__address"> <a${addAttribute(project.url, "href")} target="_blank"> <span>Go to project website</span> </a> </div>`} </div> </div> </div> </div>`;
